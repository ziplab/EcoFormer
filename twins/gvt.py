...
from performer import EcoformerFastAttention
...
class GroupBlockv2(TimmBlock):
    def __init__(
        ...
        super(GroupBlockv2, self).__init__(
            dim,
            num_heads,
            mlp_ratio,
            qkv_bias,
        ...
        if ws == 1:
            ...
            # self.attn = SRAttention(dim, num_heads, qkv_bias, qk_scale, attn_drop, drop, sr_ratio)
        else:
            self.attn = GroupAttention(
                dim, num_heads, qkv_bias, qk_scale, attn_drop, drop, ws
            )
...
from performer import EcoformerSelfAttention


class EcoBlock(TimmBlock):
    def __init__(
        self,
        dim,
        num_heads,
        ...
        if sr_ratio == 1:
            ...
            self.attn = EcoformerSelfAttention(
                dim,
                num_heads=num_heads,
                qkv_bias=qkv_bias,
                qk_scale=qk_scale,
...
@register_model
def pcpvt_small_v0_ecoformer(pretrained=False, **kwargs):
    model = CPVTV2(
        patch_size=4,
        embed_dims=[64, 128, 320, 512],
        num_heads=[1, 2, 5, 8],
...
@register_model
def alt_gvt_small_sra_to_msa(pretrained=False, **kwargs):
    model = ALTGVT(
        patch_size=4,
        embed_dims=[64, 128, 256, 512],
        num_heads=[2, 4, 8, 16],
...
@register_model
def alt_gvt_small_ecoformer(pretrained=False, **kwargs):
    model = ALTGVT(
        patch_size=4,
        embed_dims=[64, 128, 256, 512],
        num_heads=[2, 4, 8, 16],
...
@register_model
def alt_gvt_base_ecoformer(pretrained=False, **kwargs):
    model = ALTGVT(
        patch_size=4,
        embed_dims=[96, 192, 384, 768],
        num_heads=[3, 6, 12, 24],
...
@register_model
def alt_gvt_large_ecoformer(pretrained=False, **kwargs):
    model = ALTGVT(
        patch_size=4,
        embed_dims=[128, 256, 512, 1024],
        num_heads=[4, 8, 16, 32],
