...
def fast_attn_hash_flops(h, w, dim, heads):
    H = heads
    N = h * w
    C = dim // heads
    Nf = 256
    ...
    num_features = 256
    n_params += C * Nf
    # qk = create_kernel(q, is_query=True)
    macs += H * N * Nf * C + 2 * H * N * C + 2 * H * N * Nf

    Nf = num_hashes

    # random hash projection
    macs += H * N * num_features * num_hashes
    ...
    # torch.einsum('...ne,...n->...ne', out, D_inv)
    macs += H * N * Nf * C

    return macs, adds + macs
...
def main(args):
    ...
    attn_flops = {
        "msa": msa_flops,
        "performer_random_hash": fast_attn_hash_flops,
    }
